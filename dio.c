        ...
        if (*line == '[') {
            ...
            for (const char *rElem = jArrayGet(resp, 0); rElem; rElem = jArrayNext(rElem)) {
                ...
                for (const char *p = rElem; (p = jNextKeyVal(p, &key, &klen, &val)); ) {
                    switch (klen) {
                    case 2: if (!memcmp(key, "id",     2)) {
                            id        = jUint(val);
                    } break;
                    case 5: if (!memcmp(key, "error",  5)) {
                            errField  = val;
                    } break;
                    case 6: if (!memcmp(key, "result", 6)) {
                            resultVal = val;
                    } break;
                    }
                }
            ...
            for (const char *p = line; (p = jNextKeyVal(p, &key, &klen, &val)); ) {
                switch (klen) {
                case 4:
                    if (!memcmp(key, "logs", 4)) {
                        r->logs = jValDup(val);
                    }
                    break;
                case 6:
                    if (!memcmp(key, "status", 6)) {
                        r->status = jStrDup(val);
                    }
                    break;
                case 7:
    ...
    subprocess_t  *sub)
{
    ...
    for (const char *p = callJson; (p = jNextKeyVal(p, &key, &klen, &val)); ) {
        ...
        case 2:
            if (!memcmp(key, "to",    2)) {
                if (jStr(val, tmp, sizeof(tmp)) > 0) {
                    normalizeAddr(tmp, r->to);
                }
            }
            break;
        case 4:
            ...
            break;
        case 5:
            if      (!memcmp(key, "value", 5)) {
                jStr(val, r->value, sizeof(r->value));
            }
            else if (!memcmp(key, "block", 5)) {
                jStr(val, r->block, sizeof(r->block));
            }
            else if (!memcmp(key, "input", 5)) {
                dataField = val;
            }
            break;
        }
    ...
    if (strcmp(r->block, "latest") == 0) {
        ...
        if (!resp) {
            rpcFailed(kBlockNumber);
        }
        const char *errField = jFind(resp, "error");
        if (errField) {
            rpcError(kBlockNumber, errField);
        }
...
int main(int argc, char *const argv[]) {
    ...
    while ((opt = getopt_long(argc, (char *const *)argv, "ho:", longopts, NULL)) != -1) {
        switch (opt) {
        case 'h': fputs(usage, stdout);
            return 0;
        case 'o': inlineJson = optarg;
            break;
